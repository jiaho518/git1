...
```
  ...
  - 비교 연산
    - true/false의 boolean 데이터 타입의 결과
...
### Condition Starement - if 

- 조건문, 분기문
- 조건식(비교식)의 결과값(true/false)에 따라 실행 코드를 분기하는 방식
...
- 한번에 여러 갈래로 분기하는경우
- 표현식 : 결과값이 일반 연산의 모든 결과값을 사용하는 식
...
- 같은 데이터 타입의 데이터들의 집합
- 여러개의 데이터를 배열 형태로 선언하면 동일한 대표이름 과인덱스 번호를 사용할 수 있음
...
car.push('kia');
// 출력
  - console.log(car[0]); = 볼보
  - console.log(car[2]); = Saab
// 수정
  - car[1] = 'benz';
// 삭제
car.pop();
...
- 변수가 적용(인식)되는 범위
- block, function 범위는 해당 범위에서만 적용 가능
- global 범위는 전체범위에서 적용 가능
  - Block Scope
  - Function Scope
  - Global Scope
...
- 웹 페이지 / 웹 서비스에서 특정 상태의 변화
- 이벤트 발생 -> 이벤트 감지 -> 해당 이벤트에 대해 기능 실행
- 웹 페이지 상태 변화 : 이벤트
