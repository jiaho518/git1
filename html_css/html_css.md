...
```
div{
  backgroud-color:rgba(200,156,50,0.6);
}
```
...
- 부모요소에 적용하는 flex 관련 style
  - flex-direction
    - 배치 방향
    - column, column-reverse(역방향), row(기본), row-reverse
  - flex-wrap
    - 배치 줄바꿈
    - wrap, nowrap(기본)
  - jusfify-content
  ...
  - aling-itmes
    ...
    - stretch(기본) : 세로 길이가 부모요소에 맞춰서 채워짐
...
- 적응형 웹
  - OSMU 형태로 사용되지 않음
  - pc 사이트와 mobile 사이트가 분리되어 있고 디바이스 종류에 따라서 해당 사이트로 리다이렉트 됨

- 뷰포트
  - pc 해상도를 기준으로 모바일 해상도를 pc 해상도에 맞춰줌
  - 픽셀 밀도를 같도록 맞춰줌

- 픽셀 밀도
 - 모바일 디바이스 의 화면 크기가 작아서 픽셀의 크기를 작게해서 더 많은 수의 픽셀이 표현될 수 있또록 밀도를 높인 것
 - 구현
